...
class ObjectTests {
	...
	void arithmeticTest() {
		...
		assertEquals(res, Strings.computeArithmeticExpression("(a*b-(c/(2.0-d)))", new double[] {2.0, 2.0, 8.0, 2.5}, new String[] {"a", "b", "c", "d"}));
				res = 20.0;
		assertEquals(res, Strings.computeArithmeticExpression("$+2-$d/__*10", new  double[]{5, 1, 3}, new String[]{"$", "$d", "__"}));
		...
		assertTrue(Double.isNaN(Strings.computeArithmeticExpression("10 + 2 / a", new double[]{2}, new String[]{"a1"})));
		assertTrue(Double.isNaN(Strings.computeArithmeticExpression("$+2-$d/__*10", new  double[]{5, 1, 3}, new String[]{"$", "$d", "___"})));
	} 
